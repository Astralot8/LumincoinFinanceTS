...
export class Expenses {
  ...
  private showRecords(profitArray: OperationType[]): void {
    ...
    if (profitArray) {
      for (let i = 0; i < profitArray.length; i++) {
        ...
        cardButtonEditElement.href = "/expenses-edit?id=" + profitArray[i].id;
    ...
    const cardButtonCreateElement: HTMLAnchorElement | null =
      document.createElement("a");
    cardButtonCreateElement.href = "/expenses-create";
  ...
  private activateDeleteButton(): void {
    const deleteButtons: NodeListOf<HTMLAnchorElement> =
          ...
          if (this.popUpElement && this.confirmButton) {
            ...
            this.confirmButton.addEventListener("click", () => {
              ...
              if (this.confirmButton) {
                this.confirmButton.href = "/expenses-delete?id=" + id;
              }
            });
          }
        });
